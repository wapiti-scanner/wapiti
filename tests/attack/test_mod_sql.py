...
async def test_max_attack_time_5():
    ...
    respx.get(url__regex=r"http://perdu\.com/\?vuln1=.*%C2%BF%27%22%28.*&vuln2=there&vuln3=toto").mock(
        return_value=httpx.Response(
            200,
            text=(
                "You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version "
    ...
    respx.get(url__regex=r"http://perdu\.com/\?vuln1=hello&vuln2=.*%C2%BF%27%22%28.*").mock(
        side_effect=delayed_response()
    )

    respx.get(url__regex=r"http://perdu\.com/\?vuln1=.*&vuln2=.*&vuln3=.*%C2%BF%27%22%28.*").mock(
        return_value=httpx.Response(
            200,
            text=(
                "You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version "
    ...
    respx.get("http://perdu.com/?vuln1=hello&vuln2=there&vuln3=toto").mock(
        return_value=httpx.Response(200, text="Hi there")
    )

    persister = AsyncMock()

    request = Request("http://perdu.com/?vuln1=hello&vuln2=there&vuln3=toto")
    request.path_id = 1

...
async def test_persister_basic():
    ...
    persister = SqlPersister("postgresql+asyncpg://postgres:wapiti@127.0.0.1:5432/wapiti", "test1_")
    ...
    await persister.create()
    await persister.set_to_browse([simple_post])
    # We get the ID of the original request. It would go back to 1 for sqlite but will continue to increase for MySQL
    # as it is tied to a sequence generated by sqlalchemy
    request_id = [__ async for __ in persister.get_to_browse()][0].path_id
...
async def test_persister_upload():
    persister = SqlPersister("postgresql+asyncpg://postgres:wapiti@127.0.0.1:5432/wapiti",  "test2_")
    await persister.create()
    await persister.set_root_url("http://httpbin.org/")
...
async def test_persister_forms():
    with open("tests/data/forms.html") as data_body:
        ...
        persister = SqlPersister("postgresql+asyncpg://postgres:wapiti@127.0.0.1:5432/wapiti",  "test3_")
        await persister.create()
        await persister.set_root_url("http://httpbin.org/")
...
async def test_raw_post():
    ...
    persister = SqlPersister("postgresql+asyncpg://postgres:wapiti@127.0.0.1:5432/wapiti",  "test4_")
    await persister.create()
    await persister.set_root_url("http://httpbin.org/")

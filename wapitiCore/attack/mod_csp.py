...
class mod_csp(Attack):
    ...
    def check_default_src(self, response, element, check_list):
        """
        ...
        if not re.search(element, response.headers['Content-Security-Policy']):
            return -1

        # If the tested element is default-src or script-src,
        # we must ensure that none of this unsafe values are present in the CSP
        elif element in ["default-src", "script-src"]:
            if any(el in response.headers['Content-Security-Policy'] for el in check_list):
                return 1
        # If the tested element is none of the previous list,
        # we must ensure that one of this safe values is present in the CSP
        else:
            if any(elm in response.headers['Content-Security-Policy'] for elm in check_list):
                return 2
            else:
                return 1
    ...
    def attack(self):
        ...
        if "Content-Security-Policy" not in response.headers:
            self.log_red(Additional.MSG_CSP_MISSING_HEADER)
            self.add_addition(
                category=Additional.INFO_CSP,
                level=Additional.LOW_LEVEL,
                request=request,
                info=Additional.MSG_CSP_MISSING_HEADER
            )
        else:
            ...
            for def_src in self.checks:
                ...
                if result == -1:
                    self.log_red(Additional.MSG_CSP_MISSING.format(def_src))
                    self.add_addition(
                        category=Additional.INFO_CSP,
                        level=Additional.LOW_LEVEL,
                        request=request,
                        info=Additional.MSG_CSP_MISSING.format(def_src)
                    )
                elif result == 1:

...
def validate_cms_choices(cms_value):
    allowed_cms = ["drupal", "joomla", "magento", "prestashop", "spip", "typo3", "wp"]
...
def build_attack_options_from_args(args: Namespace) -> dict:
    ...
    if args.cms:
        allowed_cms = ["drupal", "magento", "joomla", "prestashop", "spip", "typo3", "wp"]
...
async def wapiti_main():
    ...
    try:
        ...
        if "jwt" in args:
            wap.add_custom_header("Authorization", f"Bearer {args.jwt}")
